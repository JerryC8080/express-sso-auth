 ...
 * Private Method
 */
function _generateToken(user, clientName) {
  // get key of client
  let key = opt.keys[clientName];
...
function _setTokenIntoCookie(res, ssoToken) {
...
function _setUserIntoCookie(req, res, clientName, callback) {
...
function _clearTokenFromCookie(req, res) {
  if (opt.token.cookie){
    res.clearCookie(opt.token.cookie.name, {
      domain: opt.token.cookie.domain || undefined,
    });
  }
  return true;
    ...
    ((callback) => {
      ...
      if (false){
        ...
        opt.middleware.beforeSerializeUser(req, res, (err, user, client) => {
          ...
          ssoToken = _generateToken(user, client);
...
module.exports = function (options) {
  ...
  return {
    ...
    api: {
      ...
      setTokenIntoCookie: _setTokenIntoCookie,
      setUserIntoCookie: _setUserIntoCookie,
      clearTokenFromCookie: _clearTokenFromCookie
    }
  }
};